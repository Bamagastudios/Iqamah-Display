...
const GRID: CSSProperties = { display: 'grid', gridTemplateColumns: '1fr 158px 172px', alignItems: 'center' };
        ...
        return (
          ...
          >
            <div style={{ display: 'flex', alignItems: 'baseline', gap: 14 }}>
              <span style={{ font: `600 36px ${font.display}`, color: color.plaster }}>{r.name}</span>
              <span dir="rtl" lang="ar" style={{ font: `700 27px ${font.arabic}`, color: color.plasterDim }}>
                {r.arabic}
              </span>
            </div>
            <span style={numStyle(30, color.plasterDim)}>{r.adhan ?? r.single ?? '—'}</span>
            <span style={numStyle(active ? 35 : 33, active ? color.brass : r.iqamah ? color.plaster : color.plasterDim)}>
              {r.iqamah ?? '—'}
            </span>
          </div>
        );
